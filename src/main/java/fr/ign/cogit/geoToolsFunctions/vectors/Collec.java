...
public class Collec {
  ...
  //// Geometry g = w.read("MULTIPOLYGON (((937159.28 6688272.91, 937171.18 6688277.86, 937175.76 6688271.59, 937192.91 6688247.08, 937194.51 6688244.77, 937210.81 6688220.7,
  // 937272.28 6688116.74, 937259.65 6688107.96, 937259.64 6688107.96, 937230.38 6688159.07, 937200.23 6688210.7, 937193.91 6688221.42, 937181.49 6688240.06, 937163.63 6688265.93,
  // 937159.28 6688272.91)))");
  // Geometry g = w.read("MultiPolygon(((937183.97284507064614445 6688228.66999999899417162, 937203.95069014118053019 6688237.79283098503947258, 937210.53298591589555144
  // 6688230.05574647802859545, 937190.5551408453611657 6688216.66019718162715435, 937183.97284507064614445 6688228.66999999899417162)))");
  ...
  public static File exportSFC(SimpleFeatureCollection toExport, File fileOut, boolean overwrite) throws IOException {
    if (toExport.isEmpty()) {
      System.out.println(fileOut.getName() + " is empty (nothing to export 127)");
      return fileOut;
    }
    ...
    File datFile = exportSFC_FeatureWriter(toExport, fileOut, toExport.getSchema());
  ...
  public static File exportSFC(SimpleFeatureCollection toExport, File fileOut) throws IOException {
    if (toExport.isEmpty()) {
      System.out.println(fileOut.getName() + " is empty (nothing to export 161)");
      return fileOut;
    }
    return exportSFC_FeatureWriter(toExport, fileOut, toExport.getSchema());
  }
   ...
   * @throws IOException
   */
  public static File exportSFC(SimpleFeatureCollection toExport, File fileOut, SimpleFeatureType ft) throws IOException {
   ...
   * @throws IOException
   */
  public static File exportSFC_FeatureWriter(SimpleFeatureCollection toExport, File fileOut, SimpleFeatureType ft) throws IOException {
    FileDataStoreFactorySpi dataStoreFactory = FileDataStoreFinder.getDataStoreFactory("shp");
    if (!fileOut.getName().endsWith(".shp")) {
      fileOut = new File(fileOut + ".shp");
    }
    Map<String, Serializable> params = new HashMap<>();
    ...
    Transaction transaction = new DefaultTransaction("create");
    String typeName = newDataStore.getTypeNames()[0];
    FeatureWriter<SimpleFeatureType, SimpleFeature> writer = newDataStore.getFeatureWriter(typeName, transaction);
    try {
      SimpleFeatureCollection features = toExport.subCollection(new Filter() {
        @Override
        public boolean evaluate(Object object) {
  ...
  public static boolean isSchemaContainsAttribute(SimpleFeatureType schema, String attributeFiledName) {
    if (schema.getAttributeDescriptors().stream().filter(s -> s.getName().toString().equals(attributeFiledName)).collect(Collectors.toList()).size() == 0)
      return false;
    else
      return true;
  }
  ...
  /**
   * Get the {@link SimpleFeature} out of a {@link SimpleFeatureCollection} that intersects a given Geometry (that is most of the time, a parcel or building). If the given feature
   * is overlapping multiple SimpleFeatureCollection's features, we calculate which has the more area of intersection.
   * 
   * @param geometry
   *          input {@link Geometry}
...
//    System.out.println("reduced = "+givenFeatureGeom);
    ...
    if (collec.isEmpty()) {
//      System.out.println("intersection between " + geometry + " and " + inputSFC.getSchema().getName() + " null");
      return null;
    }
